...
// src/lib/supabase.ts
var supabaseUrl = "https://ilzwnfwmpmasjkllpffa.supabase.co";
var supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlsenduZndtcG1hc2prbGxwZmZhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyMzY4NzAsImV4cCI6MjA5NDgxMjg3MH0.dKYil04F4Vu2ht9fZ4ZfdA0d6IlaNk5aEC5ZMSPsEyY";
var supabase = createClient(supabaseUrl, supabaseAnonKey);

// src/lib/api.ts
var apiFetchProxy = async (path, options = {}) => {
  const method = (options.method || "GET").toUpperCase();
  const body = options.body ? JSON.parse(options.body) : null;
  const token = options.headers?.Authorization?.split(" ")[1] || null;
  const res = (data, status = 200) => ({
    ok: status >= 200 && status < 300,
    status,
    json: async () => data
  });
  const errorRes = (msg, status = 400) => res({ error: msg }, status);
  try {
    if (path === "/login" && method === "POST") {
      const { data, error } = await supabase.from("users").select("*").eq("username", body.username).eq("password", body.password);
